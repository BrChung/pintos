...
*/
void
wake_up_sleeping_threads(int64_t since_boot)
{
  struct list_elem* e;
...
*/
bool
cmp_threads_priorities(const struct list_elem* a, const struct list_elem* b,
                       void* aux)
{
...
thread_update_priority(struct thread* t, int new_priority)
{
  ...
  // Yield eventually
  struct list_elem* max = list_max(&ready_list, cmp_threads_priorities, NULL);
  struct thread* t_max = list_entry(max, struct thread, elem);

  if (is_thread(t) && (t_max->priority > t->priority) &&
      !(t_max->status == THREAD_RUNNING)) {
    thread_yield();
  }
}
...
next_thread_to_run(void)
{
  if (list_empty(&ready_list)) {
    ...
    struct list_elem* max_elem =
      list_max(&ready_list, cmp_threads_priorities, NULL);
